...
import {
  body, container, ctaCopper, ctaInk, eyebrow, headLink,
  intro, lede, pageHeading, sectionHeading, sectionRule,
} from '../styles'

// Teaser set of client marks (Portfolio has all 19); Rohas omitted, no matching vector art yet
const CLIENT_MARKS = [
...
export default function Realty() {
  return (
    <>
      ...
      {/* Why this model */}
      <section id="why" className="rw-pad" style={{ ...container, padding: 'clamp(90px,11vw,130px) 40px' }}>
        <div className="rw-split" style={{ display: 'grid', gridTemplateColumns: '.36fr .64fr', gap: 56 }}>
          ...
          <div>
            ...
            <Reveal as="p" delay={200} style={{ marginTop: 24, fontFamily: serif, fontStyle: 'italic', fontSize: 'clamp(16px,1.7vw,18px)', lineHeight: 1.55, color: 'var(--ink)', maxWidth: '35em' }}>
              Team Rajiv Williams operates primarily on projects with serious inventory, long-term developer intent, and real ambition. This is not for everyone — and that is by design.
            </Reveal>
          </div>
        </div>
      </section>

      {/* Territory map — shared "projects behind the numbers" section, same as Portfolio */}
      <ProjectsSection />

      {/* Selected clients teaser — projects we hold photography for, not the mandate roster */}
      <section id="clients" style={{ ...sectionRule, borderBottom: '1px solid var(--line)', background: 'var(--chip)' }}>
        <div className="rw-pad" style={{ ...container, padding: 'clamp(80px,10vw,110px) 40px' }}>
          ...
          />
          {/* renderItem lets each tile pick light or dark treatment per mark; fade matches section bg */}
          <Reveal>
            <LogoLoop
              ...
              renderItem={(item) => (
                <div className={`rw-loop-tile${item.dark ? ' is-dark' : ''}`}>
                  {/* Eager load: lazy loading would make logos pop in mid-loop */}
                  <img
                    src={item.src}
                    alt={item.alt}
                    draggable={false}
      ...
      <ClosingCTA title="Mandates are limited to a handful of projects at a time.">
        <BookButton interest="RW Realty mandate" style={ctaInk}>DISCUSS A MANDATE</BookButton>
      </ClosingCTA>
